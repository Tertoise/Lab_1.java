...
//    }
    ...
    public void matrix_multi(double[][] matrix) throws Exception {
        if(cur_matrix[0].length != matrix.length){
            throw new Exception("Matrix are not compatible");
        }
        double[][] resulting = new double[cur_matrix.length][matrix[0].length];

        for(int i=0;i<cur_matrix.length;i++){
            for(int j=0;j<matrix[0].length;j++){
                resulting[i][j]=0;
                for(int k=0;k<cur_matrix[0].length;k++){
                    resulting[i][j]+=cur_matrix[i][k]*matrix[k][j];
                }
            }
        }
